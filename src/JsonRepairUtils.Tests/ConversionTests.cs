...
namespace JsonRepairUtils.Tests;

[TestFixture]
public class JsonRepairTests
{
    private readonly JsonRepair _jsonRepair = new();

    [SetUp]
    public void Setup()
    {
        // Enable throwing exceptions
        _jsonRepair.ThrowExceptions = true;
    }

    // Test helper function
    private void AssertRepair(string input, string output)
    {
        var repairedJson = _jsonRepair.Repair(input);
        LogAssertRepair(input,repairedJson, output);
        Assert.AreEqual(output, repairedJson);
    }
    ...
    {
        Assert.AreEqual(_jsonRepair.Repair("\"★\""), "\"★\"");
        Assert.AreEqual(_jsonRepair.Repair("\"\\u2605\""), "\"\\u2605\"");
        Assert.AreEqual(_jsonRepair.Repair("\"😀\""), "\"😀\"");
        Assert.AreEqual(_jsonRepair.Repair("\"\\ud83d\\ude00\""), "\"\\ud83d\\ude00\"");
        Assert.AreEqual(_jsonRepair.Repair("\"йнформация\""), "\"йнформация\"");
    }
    ...
    public void ParseValidJson_SupportsEscapedUnicodeCharactersInString()
    {
        ...
        Assert.AreEqual(_jsonRepair.Repair("\"\\ud83d\\ude00\""), "\"\\ud83d\\ude00\"");
        Assert.AreEqual(
            _jsonRepair.Repair("\"\\u0439\\u043d\\u0444\\u043e\\u0440\\u043c\\u0430\\u0446\\u0438\\u044f\""),
            "\"\\u0439\\u043d\\u0444\\u043e\\u0440\\u043c\\u0430\\u0446\\u0438\\u044f\""
        );
    }
    ...
    {
        Assert.AreEqual(_jsonRepair.Repair("{\"★\":true}"), "{\"★\":true}");
        Assert.AreEqual(_jsonRepair.Repair("{\"\\u2605\":true}"), "{\"\\u2605\":true}");
        Assert.AreEqual(_jsonRepair.Repair("{\"😀\":true}"), "{\"😀\":true}");
        Assert.AreEqual(_jsonRepair.Repair("{\"\\ud83d\\ude00\":true}"), "{\"\\ud83d\\ude00\":true}");
    }
    ...
    public void RepairInvalidJson_ShouldStripJsonpNotation()
    {
        ...
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("callback {}"), "Unexpected character \"{\", position: 9");
    }
    ...
    public void RepairInvalidJson_ShouldStripMongoDbDataTypes()
    {
        ...
        Assert.AreEqual(_jsonRepair.Repair(mongoDocument), expectedJson);
    }
    ...
    public void RepairInvalidJson_ShouldRepairNewlineSeparatedJson()
    {
        ...
        Assert.AreEqual(_jsonRepair.Repair(text), expected);
    }
    ...
    {
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair(""), "Unexpected end of JSON string, position: 0");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("{\"a\","), "Colon expected, position: 4");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("{:2}"), "Object key expected, position: 1");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("{\"a\":2,]"), "Unexpected character \"]\", position: 7");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("{\"a\" ]"), "Colon expected, position: 5");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("{}}"), "Unexpected character \"}\", position: 2");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("[2,}"), "Unexpected character \"}\", position: 3");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("2.3.4"), "Unexpected character \".\", position: 3");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("2..3"), "Invalid number '2.', expecting a digit but got '.', position: 2");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("2e3.4"), "Unexpected character \".\", position: 3");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("[2e,]"), "Invalid number '2e', expecting a digit but got ',', position: 2");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("[-,]"), "Invalid number '-', expecting a digit but got ',', position: 2");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("foo ["), "Unexpected character \"[\", position: 4");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("\"\\u26\""), "Invalid unicode character \"\\u26\", position: 1");
        Assert.Throws<JsonRepairError>(() => _jsonRepair.Repair("\"\\uZ000\""), "Invalid unicode character \"\\uZ000\", position: 1");
    }
}
